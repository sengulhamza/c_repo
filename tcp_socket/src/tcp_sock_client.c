...
#define TCP_SOCK_CLIENT_BUFFER_SIZE 1024

static void *tcp_sock_thread_send(void *cfd)
{
    char tmp[TCP_SOCK_CLIENT_BUFFER_SIZE] = {"A"};
    int c = *((int *)cfd);
    while (1) {
        send(c, tmp, TCP_SOCK_CLIENT_BUFFER_SIZE, 0);
        printf("        send ok\n");
        sleep(2);
    }
}
...
{
    char tmp[TCP_SOCK_CLIENT_BUFFER_SIZE];
    int d = *((int *)cfd);
    while (1) {
        int idata = 0;
        idata = recv(d, tmp, TCP_SOCK_CLIENT_BUFFER_SIZE, 0);
        if (idata > 0) {
            printf("%s \r\n", tmp);
        }
    }
...
int main()
{
    ...
    serv.sin_port = htons(TCP_SOCK_CLIENT_PORT);
